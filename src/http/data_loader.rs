...
use crate::http::data_loader_type::{DataLoader, Loader}; 
...
// eliminate this implementation detail all together here...
...
impl Loader<DataLoaderRequest> for HttpDataLoader {
  ...
  async fn load(
  ...
  ) -> async_graphql::Result<FxHashMap<DataLoaderRequest, Self::Value>, Self::Error> {
    if let Some(group_by) = self.batched.clone() {
      ...
      let mut hashmap: FxHashMap<DataLoaderRequest, Response> = FxHashMap::default();
